...
def run_daily_report(rtype, cdate):
    try:
        # משיכת השעה המדויקת בישראל ברגע שליחת הדו"ח
        ...
        for cid, symbols in users.items():
            # כותרת דינמית: תלויה במה שהגיע מה-Cron Job
...
def run_sniper():
    try:
        ...
        # קיבוץ משתמשים לפי מניות כדי לא לסרוק את אותה מניה פעמיים
        symbols_to_users = {}
        for e in data:
            sym = e['symbol'].upper()
        ...
        for sym, users_data in symbols_to_users.items():
            ...
            for user_row in users_data:
                ...
                if (abs(curr_c) >= 5 and last_p == 0) or (abs(curr_c - last_p) >= 7):
                    ...
                    # עדכון בסיס הנתונים הספציפי למשתמש הזה כדי למנוע כפילויות
                    supabase.table('user_preferences').update({'last_alert_percent': curr_c}).eq('id', user_row['id']).execute()
            
            time.sleep(2) # השהייה בין מניה למניה כדי לשמור על ה-API
...
# ראוט הדו"ח החכם שיודע לקבל Opening Bell או Closing Bell
